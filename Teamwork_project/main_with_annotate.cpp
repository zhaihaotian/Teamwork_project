...
struct node
{
	...
	string nam ;       //  这个站点的名字   
    string route;      // 这个站点所在的线路 
}mpp[maxn];

unordered_map<string, vector<int> > mp;      // 表示这个站点一共有哪几个点                     
unordered_map<int, unordered_map<int, double>> dis , tim; // 表示两点间的距离，时间
string s1, s2, temp = "0";
int last;
unordered_map<string, double> v , change_tim; // v是线路的配速  change_tim 是这个站点换乘的时间 
double a, b, xx, yy;
int id_cnt = 481; 
...
// 建了两个图 一个是正常的 另一个是少换乘的图 

double change_k = 8  ;  // 在少换乘的图中  我们其实就是把换乘时间的权重放大 
double ka = 1 , kb = 1 ; // 二人一起出发的权重 当ka/kb的值越大 意味你们想一起走的路就越多 
